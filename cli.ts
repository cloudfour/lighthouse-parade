...
sade('lighthouse-parade <url> [dataDirectory]', true)
  ...
  .option(
    '--path-must-match',
    'Specify a glob (in quotes) for paths to match. Links to non-matched paths will not be crawled. This flag can be specified multiple times to allow multiple paths. `*` matches one url segment, `**` matches multiple segments. Trailing slashes are ignored.'
  )
  .option(
    '--path-must-not-match',
    'Specify a glob (in quotes) for paths to exclude. Links to matched paths will not be crawled. This flag can be specified multiple times to exclude multiple paths. `*` matches one url segment, `**` matches multiple segments. Trailing slashes are ignored.'
  )
  .action(
    (
      url,
